...
class CeilingFanAccessory {
    constructor(platform, accessory) {
        ...
        this.fanService.getCharacteristic(this.platform.Characteristic.On)
            .onSet(async (value) => {
            this.state.fanOn = value.valueOf();
        ...
        this.fanService.getCharacteristic(this.platform.Characteristic.RotationDirection)
            .onSet(async (value) => {
            this.state.fanRotation = value.valueOf();
            await device.set({ dps: 8, set: this.state.fanRotation === 0 ? 'forward' : 'reverse', shouldWaitForResponse: false });
        })
            .onGet(() => this.state.fanRotation);
        const rotationHook = (data) => {
        ...
        this.fanService.getCharacteristic(this.platform.Characteristic.RotationSpeed)
            .onSet(async (value) => {
            if (value.valueOf() === 0) {
                await device.set({ dps: 16, set: false, shouldWaitForResponse: false });
            }
            else {
                this.state.fanSpeed = value.valueOf();
                await device.set({ dps: 3, set: this.toStep(this.state.fanSpeed), shouldWaitForResponse: false });
            }
        })
            .onGet(() => this.state.fanSpeed)
            .setProps({});
        const speedHook = (data) => {
        ...
        if (accessory.context.device.hasLight) {
            ...
            this.lightService.getCharacteristic(this.platform.Characteristic.On)
                .onSet(async (value) => {
                this.state.lightOn = value.valueOf();
            ...
            this.lightService.getCharacteristic(this.platform.Characteristic.Brightness)
                ...
                if (value.valueOf() === 0) {
                    ...
                    this.state.lightBrightness = value.valueOf();
                    await device.set({ dps: 16, set: this.state.lightBrightness * 10, shouldWaitForResponse: false });
                }
            })
                .onGet(() => this.state.lightBrightness);
            const lightBrightnessHook = (data) => {
    ...
    toPercent(initialPercentage, step) {
    ...
    convertTemperatureHomeKit(tuyaValue) {
        switch (tuyaValue) {
            case 0:
                return 140;
            case 500:
    ...
    convertTemperatureTuya(homekitValue) {
